...
def _validate_observed_facts(
    ...
    *,
    ...
    elif "project_id" in observed:
        raise CodexSessionObservationError("project_id mismatch")
    if repository_binding is not None:
        expected["canonical_cwd"] = os.path.realpath(repository_binding.cwd)
    elif "canonical_cwd" in observed:
        raise CodexSessionObservationError("canonical_cwd mismatch")
    missing = set(expected) - keys
    if missing:
        raise CodexSessionObservationError(f"missing observed fact {sorted(missing)[0]}")
