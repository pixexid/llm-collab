...
> plus a server-derived
> `safeToStart` boolean); it adds no UI, provider call, scheduling, delivery, or publication. The P4
> affordance remains separate. The accepted Lane B contract in §§2, 3, and 5.2–5.3 remains implementation authority.
...
- **`amiga-operator-cmo`**: IMPACTED as workflow knowledge. S1 voice guide, Lane B draft generation
  ...
  GH-1515 adds server authority only, not a bot tool or generation affordance: until P4 ships, the bot
  must still not direct an operator to a UI control that does not exist.
