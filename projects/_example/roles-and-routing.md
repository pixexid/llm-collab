...
- Real project-specific identities, repository paths, customer names, queue state, and operational details belong only in your local `{project_state_root}/<project_id>/` directory.

## Queue Rule

If the project has an ordered queue, keep it in local `{project_state_root}/<project_id>/issue-queue.json` and regenerate `issue-queue.md` with:
