...
class ReviewLoopCapContractTest(unittest.TestCase):
    ...
    def test_the_contract_version_and_recent_summaries_stay_current(self):
        ...
        self.assertIn("<!-- CONTRACT_VERSION: 16 -->", text)
        ...
        for phrase in (
            ...
            "Session Startup",
            "Contract v15",
            "hard exclusions for every text-bearing assignment",
            "read-only included",
            "evidence that gates later decisions",
            "no diff review",
            "changed activation eligibility",
            "BB Workers",
            "Contract v14",
            "send-time routing admission fail closed before durability",
            "two packets to Claude",
            "both carried `target_session_id: null`",
            "reading the chat directory by hand",
            "after the sender asked why there had been no reply",
            "Message file schema",
            "Send a packet",
            "Contract v13",
            "writing lanes only",
            "Read-only lanes",
            "never counted and do not now",
            "One-writer-per-lane remains a separate, unchanged rule",
            "orchestrator verification capacity",
            "a judgment rather than a constant",
            "a full session under an override recorded only in a chat message",
            "Contract v11",
