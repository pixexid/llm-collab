...
class ProjectDesignQueueTest(unittest.TestCase):
    ...
    def test_removed_claude_prompt_commands_are_rejected(self) -> None:
        for command in ("bridge-status", "record-computer-use-timeout", "desktop-prompt"):
            with self.subTest(command=command):
                with patch.object(sys, "argv", ["project_design_queue.py", command, "--project", "amiga"]):
                    with patch("sys.stderr", new_callable=StringIO):
                        with self.assertRaises(SystemExit) as raised:
                            project_design_queue.parse_args()
