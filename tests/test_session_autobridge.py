...
class SessionAutobridgeTest(unittest.TestCase):
    ...
    def test_load_binding_rejects_malformed_and_non_object_json(self):
        ...
        with patch.object(session_autobridge_lib, "BINDINGS_DIR", binding_root):
            # GH-687: a binding that EXISTS but cannot be parsed is present-but-unreadable, not
            # absent. It must raise BindingUnreadable so the dispatch seam leaves
            # wake_fallback_allowed False. Callers dispatch on type, and FileNotFoundError means
            # "there is no such binding" -- collapsing a corrupt record into it reports the real
            # fault as "no binding" and reopens the wake fallback lane.
            for payload in ("{", "[]"):
            ...
            # Both directions: a genuinely ABSENT binding still raises FileNotFoundError and still
            # means "no binding". A refusal-only test could not tell this fix from one that treats
            # every missing binding as unreadable; this is the half that guards the absent path.
    ...
    def test_an_unreadable_binding_suppresses_the_ax_doorbell_for_codex(self):
        ...
        chat_dir = self.create_chat(
            root,
            chat_dir_name="2026-08-08_unreadable-binding__CHAT-UNREAD-DOORBELL",
            chat_id="CHAT-UNREAD-DOORBELL",
            project_id="amiga",
        )
        # A binding that EXISTS but cannot be parsed -- the accident class (truncated write, byte
        # fault). Present, so it is not "no binding"; unreadable, so it is terminal.
        binding_path = (
            root
            / "State"
            / "session_autobridge"
        ...
        payload, stderr = self.deliver_with_scope(
            root, "CHAT-UNREAD-DOORBELL", recipient="codex", sender="claude",
            expected_returncode=2)

        self.assertFalse(payload["autobridge_ready"])
        self.assertTrue(payload.get("binding_unreadable_blocker"),
                        "a corrupt binding must surface as unreadable, not as absent")
        self.assertNotEqual(
            "exact_binding_required", payload["autobridge_refusal_reason"],
            "a corrupt binding must not be reported as absent (exact_binding_required)")
        for flag in self.WAKE_FLAGS:
            self.assertFalse(payload.get(flag),
                             f"{flag} must be false for an unreadable binding, got {payload.get(flag)!r}")
        for prompt in self.WAKE_PROMPTS:
            self.assertIsNone(payload.get(prompt),
                              f"{prompt} must be null, got {payload.get(prompt)!r}")
        self.assertFalse(payload["watcher_pickup_ready"])
