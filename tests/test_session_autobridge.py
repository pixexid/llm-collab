...
class SessionAutobridgeTest(unittest.TestCase):
    ...
    def test_watch_inbox_event_failure_after_success_returns_receipt_without_redispatch(self):
        ...
        self.assertEqual(
            ...
            returned_actions[0]["diagnostic_errors"],
        )
        consumed = [
    ...
    def test_leased_dispatch_still_settles_before_event_failure(self):
        ...
        self.assertIn(message["path"], session["processed_messages"])
        self.assertEqual(
            "OSError", first["actions"][0]["diagnostic_errors"][0]["error_type"]
        )
        self.assertEqual([], second["actions"])
        runtime_trigger.assert_called_once_with(session, message)

    def test_mixed_poll_event_failure_returns_settled_action_and_receipt(self):
        session = {
            "session_id": "SESSION-MIXED-EVENT-FAILURE",
            "agent_id": "gemini",
            "mode": "auto-read",
            "wake_strategy": "runtime_trigger",
            "runtime": {"family": "gemini_cli", "session_id": "runtime-mixed"},
        }
        message = {"path": "Chats/mixed/accepted.md", "frontmatter": {}}
        refusal = {
            "path": "Chats/mixed/refused.md",
        ...
        def matching(
            _session,
            *,
            invocation_repo_targets=None,
            repo_scope_refusals=None,
            skip_paths=None,
        ):
            repo_scope_refusals.append(refusal)
            return [message]
        ...
        def append(_session_id, event):
            if event.get("message_path") == refusal["path"]:
                raise OSError("post-loop event fsync failed")
        ...
        with self._dispatch_patch_context(session, [message]), patch.object(
            ...
            watch_inbox_lib,
            "autobridge_session_ids",
            return_value=["SESSION-MIXED-EVENT-FAILURE"],
        ), patch.object(
            watch_inbox_lib,
            "load_session",
            return_value=session,
        ...
        self.assertEqual(1, len(returned_actions))
        self.assertEqual(
            "post-loop event fsync failed",
            returned_actions[0]["diagnostic_errors"][0]["detail"],
        )
        consumed = [
            event for event in watcher_events if event["event"] == "autobridge_consumed"
        ]
    ...
    def test_dispatch_routes_event_writes_through_settlement_seam(self):
        self.assertNotIn(
            "append_event", session_autobridge_lib.dispatch_session.__code__.co_names
        )
    ...
    def test_pre_dispatch_failure_remains_retryable(self):
        ...
        with self._dispatch_patch_context(session, [message]), patch.object(
            ...
            session_autobridge_lib,
            "append_event",
            side_effect=[OSError("pre-settlement event failure"), None],
        ):
            with self.assertRaisesRegex(OSError, "pre-settlement event failure"):
    ...
    def sender_provenance_workspace(
        ...
        *,
        ...
        self.run_cli(
            root,
            "register",
            "--session", f"SESSION-CLAUDE-{project_id.upper()}",
            "--agent", "claude",
            "--project", project_id,
            "--chat", chat_id,
            "--mode", "notify",
            "--runtime-family", "claude_app",
            "--runtime-session-id", f"claude-runtime-{project_id}",
    ...
    def test_deliver_and_inbox_surface_session_protocol_fields(self):
        ...
        self.run_cli(
            root,
            "register",
            "--session", "SESSION-CLAUDE-PROTOCOL",
            "--agent", "claude",
            "--project", "amiga",
            "--chat", "CHAT-PROTO1",
            "--mode", "notify",
            "--runtime-family", "claude_app",
            "--runtime-session-id", "claude-app-session-9",
            "--runtime-session-source", "first_read",
        )
        subprocess.run(
            [
    ...
    def test_load_binding_rejects_malformed_and_non_object_json(self):
        ...
        with patch.object(session_autobridge_lib, "BINDINGS_DIR", binding_root):
            # GH-687: load_binding is a SHARED contract, so both registered projects -- Amiga and
            # the non-Amiga Nuvyr -- must prove a binding that EXISTS but cannot be parsed is
            # present-but-unreadable, not absent. It must raise BindingUnreadable so the dispatch
            # seam leaves wake_fallback_allowed False. Callers dispatch on type, and
            # FileNotFoundError means "there is no such binding" -- collapsing a corrupt record
            # into it reports the real fault as "no binding" and reopens the wake fallback lane.
            for project in ("amiga", "nuvyr"):
                with self.subTest(project=project):
                    path = binding_root / project / "CHAT-BAD-BINDING" / "claude.json"
                    for payload in ("{", "[]"):
                        with self.subTest(payload=payload):
                            write(path, payload)
                            with self.assertRaises(session_autobridge_lib.BindingUnreadable):
                                session_autobridge_lib.load_binding(
                                    project, "CHAT-BAD-BINDING", "claude"
                                )
                    ...
                    # Both directions: a genuinely ABSENT binding still raises FileNotFoundError
                    # and still means "no binding". A refusal-only test could not tell this fix
                    # from one that treats every missing binding as unreadable; this is the half
                    # that guards the absent path.
    ...
    def test_deliver_refuses_malformed_binding_before_write(self):
        root = self.make_workspace()
        self.add_agent(
            root,
            {
        ...
        deliver_result = subprocess.run(
            [
                ...
                "amiga",
                "--title",
                "Malformed binding refuses",
                "--body-file",
                "-",
            ],
            cwd=root,
            text=True,
            input="Refuse before writing when the exact binding is unreadable.",
            capture_output=True,
            check=False,
        )
        self.assertEqual(2, deliver_result.returncode, deliver_result.stderr)
        ...
        # GH-687: a binding that EXISTS but cannot be parsed (here invalid UTF-8) is
        # present-but-unreadable, NOT absent. It must surface as the binding_unreadable blocker --
        # one of the two terminal states that suppress every wake lane -- not as
        # exact_binding_required, which would report the real fault as "no binding" and leave the
        # wake fallback open.
        self.assertTrue(result_payload["binding_unreadable_blocker"])
        self.assertTrue(
            result_payload["autobridge_refusal_reason"].startswith(
                session_autobridge_lib.BINDING_UNREADABLE_REASON),
            f"expected the unreadable refusal, got {result_payload['autobridge_refusal_reason']!r}",
        )
        ...
        for project, chat_id in (
            ...
            with self.subTest(project=project):
                ...
                self.run_cli(
                    ...
                    "--runtime-session-id", f"relay-runtime-expired-{project}",
                    "--runtime-session-source", "first_read",
                )
                session_path = (
                    root / "State" / "session_autobridge" / "sessions"
                ...
                self.assertFalse(list(chat_dir.glob("*_to-relay_*.md")))

    def test_mismatched_binding_refuses_before_durable_write(self):
        root = self.make_workspace()
        for agent in ("codex", "claude"):
            self.add_agent(
                root,
    ...
    def deliver_with_scope(self, root, chat_id, *, repo_targets=None, project="amiga",
                           recipient="claude", expected_returncode=0, sender="codex"):
        """Run deliver.py and return its JSON payload plus stderr."""
        argv = [
            sys.executable, str(DELIVER_SCRIPT),
            "--chat", chat_id, "--from", sender, "--to", recipient,
    ...
    # --- undispatchable recipients fail before the durable write -----------------------------
    ...
    def test_an_undispatchable_session_is_refused_before_write(self):
        ...
        self.assertTrue(payload["delivery_refused"])
        self.assertEqual("exact_binding_not_dispatchable", payload["routing_refusal_reason"])
        self.assertFalse(payload["durable_write"])
        self.assertFalse(list(chat_dir.glob("*_to-claude_*.md")))
    ...
    def test_watcher_backing_without_a_session_is_refused_before_write(self):
        root, chat_dir = self.scoped_subscriber_workspace(
        ...
        payload, _stderr = self.deliver_with_scope(
            root, "CHAT-SCOPE1", recipient="relay", expected_returncode=2
        )
        self.assertTrue(payload["delivery_refused"])
    ...
    def test_an_unreadable_binding_suppresses_the_ax_doorbell_for_codex(self):
        """GH-687 end-to-end: a corrupt codex binding must close the AX doorbell.

        Codex is the one recipient the AX doorbell can reach, so this is the exact dangerous
        state the issue names. Under the defect, load_binding collapsed the corrupt record into
        FileNotFoundError, resolve_exact_dispatch_pair reported exact_binding_required, and
        wake_fallback_allowed stayed True -- so the AX doorbell rang a recipient whose binding
        could not be read. With the fix the corrupt binding raises BindingUnreadable, the
        unreadable blocker is terminal, and every wake lane (the doorbell included) is closed
        before the durable write.

        load_binding and the routing that reads it are a SHARED contract, so the doorbell must
        close on BOTH the Amiga and the non-Amiga (Nuvyr) project path -- a proof that only fires
        on Amiga does not establish the shared contract.
        """
        root = self.make_workspace()
        for agent_id, activation in (
            ("codex", {"type": "cli_session", "watcher_enabled": True, "ax_app": "Codex"}),
            ("claude", {"type": "cli_session", "watcher_enabled": True}),
        ):
            self.add_agent(
                root,
                {"id": agent_id, "display_name": agent_id.title(), "activation": activation},
            )
        for project in ("amiga", "nuvyr"):
            with self.subTest(project=project):
                chat_id = f"CHAT-UNREAD-DOORBELL-{project.upper()}"
                chat_dir = self.create_chat(
                    root,
                    chat_dir_name=f"2026-08-08_unreadable-binding__{chat_id}",
                    chat_id=chat_id,
                    project_id=project,
                )
                # A binding that EXISTS but cannot be parsed -- the accident class (truncated
                # write, byte fault). Present, so it is not "no binding"; unreadable, so it is
                # terminal.
                binding_path = (
                    root
                    / "State"
                ...
                payload, stderr = self.deliver_with_scope(
                    root, chat_id, project=project, recipient="codex", sender="claude",
                    expected_returncode=2)

                self.assertFalse(payload["autobridge_ready"])
                self.assertTrue(payload.get("binding_unreadable_blocker"),
                                f"{project}: a corrupt binding must surface as unreadable, not as absent")
                self.assertNotEqual(
                    "exact_binding_required", payload["autobridge_refusal_reason"],
                    f"{project}: a corrupt binding must not be reported as absent (exact_binding_required)")
                for flag in self.WAKE_FLAGS:
                    self.assertFalse(payload.get(flag),
                                     f"{project}: {flag} must be false for an unreadable binding, got {payload.get(flag)!r}")
                for prompt in self.WAKE_PROMPTS:
                    self.assertIsNone(payload.get(prompt),
                                      f"{project}: {prompt} must be null, got {payload.get(prompt)!r}")
                self.assertFalse(payload["watcher_pickup_ready"])
                self.assertIn("REFUSED before durable write", stderr)
    ...
    def test_a_non_codex_ax_app_cannot_override_routeless_refusal(self):
        root = self.make_workspace()
        for agent, activation in (
            ...
            self.add_agent(root, {"id": agent, "display_name": agent.title(),
                                  "activation": activation})
        chat_dir = self.create_chat(root, chat_dir_name="2026-07-25_no-session__CHAT-NOSESS",
                                    chat_id="CHAT-NOSESS", project_id="amiga")
        payload, _stderr = self.deliver_with_scope(
            root, "CHAT-NOSESS", recipient="relay", expected_returncode=2
        )
        self.assertTrue(payload["delivery_refused"])
        self.assertFalse(payload["ax_doorbell_required"])
    ...
    # --- an unreadable recipient binding fails closed before the durable write ---------------
    ...
    def test_an_oversized_recipient_binding_refuses_before_write(self):
        root, chat_dir = self.scoped_subscriber_workspace(
        ...
        for project, chat_id in (("amiga", "CHAT-SCOPE-A"), ("nuvyr", "CHAT-SCOPE-N")):
            with self.subTest(project=project):
                ...
                refused, stderr = self.deliver_with_scope(
                    root, chat_id, project=project, expected_returncode=2
                )
    ...
    def test_scoped_subscriber_refuses_an_empty_packet_scope(self):
        root, _chat_dir = self.scoped_subscriber_workspace(
            subscriber_repo_targets=["llm-collab"])
        payload, stderr = self.deliver_with_scope(
            root, "CHAT-SCOPE1", expected_returncode=2
        )
        self.assertFalse(payload["autobridge_ready"],
    ...
    def test_deliver_false_readiness_refuses_before_write(self):
        root = self.make_workspace()
        self.add_agent(
            root,
            {
        ...
        write(body_file, "Refuse this packet before writing it.")
    ...
    def test_deliver_suppresses_codex_self_activation_for_amiga_and_non_amiga(self):
        ...
        for project_id, chat_id in cases:
            with self.subTest(project_id=project_id):
                ...
                self.assertIn("use its BB thread", deliver_result.stdout)
                self.assertIn("docs/workflows/bb-workers.md", deliver_result.stdout)
                self.assertNotIn("read_thread", deliver_result.stdout)
                self.assertNotIn("send_message_to_thread", deliver_result.stdout)
    ...
    def test_deliver_never_rings_a_claude_cli_session_without_a_binding(self):
        ...
        self.assertFalse(list(chat_dir.glob("*_to-claude_*.md")))
        self.assertFalse(result_payload["watcher_pickup_ready"])
        self.assertFalse(result_payload["activation_unavailable"])
    ...
    def test_non_codex_identity_cannot_get_ax_doorbell_from_spoofed_app(self):
        ...
        self.assertFalse(result_payload["durable_write"])
        self.assertFalse(list(chat_dir.glob("*_to-cdx2_*.md")))

    def test_deliver_refuses_terminal_only_cli_session_before_write(self):
        # #given
        root = self.make_workspace()
        self.add_agent(
            root,
        ...
        self.assertEqual(
            ...
            result_payload["routing_refusal_reason"],
        )
        self.assertFalse(list(chat_dir.glob("*_to-terminal-worker_*.md")))

    def test_deliver_refuses_api_trigger_without_runtime_before_write(self):
        # #given
        root = self.make_workspace()
        self.add_agent(
            root,
        ...
        self.assertFalse(result_payload["relay_required"])
        self.assertFalse(result_payload["operator_relay_required"])
        self.assertEqual("exact_binding_required", result_payload["routing_refusal_reason"])
        self.assertFalse(list(chat_dir.glob("*_to-hook_*.md")))
    ...
    def test_watcherless_human_broadcast_reports_itself_explicitly(self):
        root = self.make_workspace()
        self.add_agent(
            root,
            {
        ...
        result_payload = json.loads(deliver_result.stdout.split("\n\n", 1)[0])
        self.assertEqual("broadcast", result_payload["routing_mode"])
        self.assertFalse(result_payload["autobridge_ready"])
        self.assertEqual(
            session_autobridge_lib.EXACT_BINDING_REQUIRED_REASON,
            result_payload["autobridge_refusal_reason"],
    ...
    def test_deliver_refuses_thread_pair_as_dispatch_authority(self):
        ...
        with patch.object(
            session_autobridge_lib,
            "THREAD_PAIRS_DIR",
            root / "State" / "session_autobridge" / "thread_pairs",
        ):
            session_autobridge_lib.update_thread_pair(
                "amiga",
                "CHAT-PAIR1",
                "codex",
                "cdx2",
        ...
        self.assertEqual(2, reverse_result.returncode, reverse_result.stderr)
    ...
    def test_exact_watcher_refusal_only_is_distinguishable_from_empty(self):
        args = SimpleNamespace(
            me="claude",
    ...
    def test_exact_watcher_repeated_refusal_emits_once_until_reason_changes(self):
        ...
        refused_path = "Chats/exact/refused.md"
        stdout = StringIO()
        argv = [
            "watch_inbox.py",
            "--me",
