...
from llm_collab.codex_session_observation import CodexSessionObservationError, build_session_observation
...
def observed_facts(*, native_session_id="native-alpha", project_id=None, cwd=None, revision="r1"):
    facts = {
        "host_session_id": native_session_id,
        "endpoint_id": "endpoint_codex",
        "authority_identity": "codex_app_server",
        "implementation_revision": revision,
        "capability_profile_id": "codex_read_only",
        "capability_profile_revision": "cap_r1",
    }
    ...
    if cwd is not None:
        facts["canonical_cwd"] = os.path.realpath(cwd)
    return facts
...
class CodexSessionObservationTests(unittest.TestCase):
    def build(self, root: Path, *, project_id="proj", home_name="home", native_session_id="native-alpha", authority_revision="r1"):
        ...
        return build_session_observation(
            ...
            observed=observed_facts(native_session_id=native_session_id, project_id=project_id, cwd=cwd, revision=authority_revision),
        ).session_ref
        ...
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            with self.assertRaisesRegex(CodexSessionObservationError, "unknown observation state"):
                build_session_observation(
                    ...
                    observed=observed_facts(native_session_id="native-beta"),
                )
        ...
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            with self.assertRaisesRegex(CodexSessionObservationError, "renderer evidence"):
                build_session_observation(
                    ...
                    correlation_id="corr_workspace",
                    observation_state="idle",
                    observed=observed_facts() | {"renderer_visible": True},
                )
            ...
            with self.assertRaisesRegex(CodexSessionObservationError, "unknown observed fact"):
                build_session_observation(
                    ...
                    correlation_id="corr_workspace",
                    observation_state="idle",
                    observed=observed_facts() | {"untrusted": True},
                )

            with self.assertRaisesRegex(CodexSessionObservationError, "missing observed fact"):
                build_session_observation(
                    workspace_id="ws_alpha",
                    scope={"kind": "workspace"},
                    endpoint_id="endpoint_codex",
        ...
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            with self.assertRaisesRegex(Exception, "session_ref_id mismatch"):
                ...
                build_session_observation(
                    ...
                    observed=observed_facts(project_id="alpha", cwd=repo / "work"),
        ...
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            with self.assertRaisesRegex(Exception, "project_id mismatch"):
                ...
                build_session_observation(
                    ...
                    observed=observed_facts(project_id="beta", cwd=repo),
                )
    ...
    def test_authority_drift_is_load_bearing(self):
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            first = build_session_observation(
                workspace_id="ws_alpha",
                scope={"kind": "workspace"},
                endpoint_id="endpoint_codex",
                native_session_id="native-alpha",
                ...
                correlation_id="corr_workspace",
                observation_state="idle",
                observed=observed_facts(revision="r1"),
            ).session_ref
            second = build_session_observation(
                workspace_id="ws_alpha",
                scope={"kind": "workspace"},
                endpoint_id="endpoint_codex",
                native_session_id="native-alpha",
                ...
                correlation_id="corr_workspace",
                observation_state="idle",
                observed=observed_facts(revision="r2"),
            ).session_ref
        ...
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            for observed, pattern in (
                ...
                with self.subTest(pattern=pattern), self.assertRaisesRegex(CodexSessionObservationError, pattern):
                    build_session_observation(**kwargs, observed=observed_facts(project_id="alpha", cwd=cwd) | observed)
            ...
            for observed, pattern in (
                ({"project_id": "alpha"}, "project_id mismatch"),
                ({"canonical_cwd": os.path.realpath(cwd)}, "canonical_cwd mismatch"),
            ):
                with self.subTest(pattern=pattern), self.assertRaisesRegex(CodexSessionObservationError, pattern):
                    build_session_observation(**workspace_kwargs, observed=observed_facts() | observed)

    def test_observation_state_is_preserved_outside_session_binding(self):
        with TemporaryDirectory(dir="/tmp") as tmp:
            root = Path(tmp)
            home = root / "home"
            home.mkdir()
            idle = build_session_observation(
                workspace_id="ws_alpha",
                scope={"kind": "workspace"},
                endpoint_id="endpoint_codex",
                native_session_id="native-alpha",
            ...
            busy = build_session_observation(
                workspace_id="ws_alpha",
                scope={"kind": "workspace"},
                endpoint_id="endpoint_codex",
                native_session_id="native-alpha",
        ...
        self.assertEqual("idle", idle.observation_state)
        self.assertEqual("busy", busy.observation_state)
        self.assertEqual(idle.session_ref, busy.session_ref)
        self.assertEqual("routed", idle.session_ref["evidence"]["state"])
