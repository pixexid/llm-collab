...
from llm_collab.ledger.store import (
    ...
    _v7_schema_fingerprint_from_sql,
...
def v7_fingerprint_with_v4(statements: tuple[str, ...]) -> str:
    with closing(sqlite3.connect(":memory:", isolation_level=None)) as connection:
        for statement in (*V1_SQL, *V2_SQL, *V3_SQL, *statements, *V5_SQL, *V6_SQL, *V7_SQL):
            connection.execute(statement)
        return store_module._schema_fingerprint(connection)


def v8_fingerprint_with_v4(statements: tuple[str, ...]) -> str:
    with closing(sqlite3.connect(":memory:", isolation_level=None)) as connection:
        for statement in (
            *V1_SQL,
            *V2_SQL,
...
def v9_fingerprint_with_v4(statements: tuple[str, ...]) -> str:
    with closing(sqlite3.connect(":memory:", isolation_level=None)) as connection:
        for statement in (
            *V1_SQL,
            *V2_SQL,
...
def v10_fingerprint_with_v4(statements: tuple[str, ...]) -> str:
    with closing(sqlite3.connect(":memory:", isolation_level=None)) as connection:
        for statement in (
            *V1_SQL,
            *V2_SQL,
...
def v12_fingerprint_with_v4(statements: tuple[str, ...]) -> str:
    with closing(sqlite3.connect(":memory:", isolation_level=None)) as connection:
        for statement in (
            *V1_SQL,
            *V2_SQL,
...
def v13_fingerprint_with_v4(statements: tuple[str, ...]) -> str:
    with closing(sqlite3.connect(":memory:", isolation_level=None)) as connection:
        for statement in (
            *V1_SQL,
            *V2_SQL,
...
def v14_fingerprint_with_v4(statements: tuple[str, ...]) -> str:
...
def open_mutated_v4(paths: LedgerPaths, statements: tuple[str, ...]):
    ...
    v7_fingerprint = v7_fingerprint_with_v4(statements)
    ...
    with (
        ...
        patch.object(store_module, "V7_SCHEMA_FINGERPRINT", v7_fingerprint),
...
class LedgerStoreTest(unittest.TestCase):
    ...
    def test_writer_releases_lock_when_fd_proof_is_unavailable(self) -> None:
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            with LedgerStore.open_writer(paths) as writer:
                self.assertEqual(writer.schema_version(), SCHEMA_VERSION)
    ...
    def test_all_file_backed_connects_use_one_verified_noncreating_open(self) -> None:
        ...
        self.assertIn("def _v12_schema_fingerprint_from_sql", source)
    ...
    def test_v5_schema_connection_guards_backups_and_private_permissions(self) -> None:
        ...
        self.assertEqual(sum("CREATE TABLE observation_scheduler_cursors" in sql for sql in V7_SQL), 1)
        self.assertEqual(sum("CREATE TABLE conversation_" in sql for sql in V8_SQL), 2)
        self.assertEqual(sum("CREATE TABLE lifecycle_provider_registry" in sql for sql in V8_SQL), 1)
        ...
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    "ledger-6-20260721T080506123456Z.sqlite3",
            ...
            self.assertEqual(_migration_checksum(V7_SQL), V7_MIGRATION_CHECKSUM)
            self.assertEqual(_v7_schema_fingerprint_from_sql(), V7_SCHEMA_FINGERPRINT)
            self.assertEqual(_migration_checksum(V8_SQL), V8_MIGRATION_CHECKSUM)
            self.assertEqual(_v8_schema_fingerprint_from_sql(), V8_SCHEMA_FINGERPRINT)
            self.assertEqual(_migration_checksum(V9_SQL), V9_MIGRATION_CHECKSUM)
            ...
            self.assertEqual(_migration_checksum(V12_SQL), V12_MIGRATION_CHECKSUM)
    ...
    def test_v11_to_v12_migration_preserves_legacy_session_ref_and_backfills_agent(self) -> None:
        with TemporaryDirectory(dir="/tmp") as tmp:
            paths = LedgerPaths.derive(tmp, "ws_alpha")
            paths.ensure_directories()
            stamp = FIXED_TIME.strftime("%Y%m%dT%H%M%S%fZ")
            applied_at = FIXED_TIME.isoformat()
            with closing(sqlite3.connect(paths.ledger)) as connection, connection:
                connection.execute("PRAGMA foreign_keys = ON")
                for _version, statements in store_module.MIGRATIONS[:11]:
                ...
                connection.execute(
                    ...
                    VALUES ('ws_alpha', 'project', 'amiga', 'CHAT-SAMEID', 'participant_codex', 'agent_codex', ?)
                    """,
                    (applied_at,),
                )
                connection.execute(
                    ...
                    VALUES ('ws_alpha', 'provider_codex', 'revision_1', 'managed',
                            '["reserve","attach"]', 'sha256', 60, ?)
                    """,
                    (applied_at,),
                )
                connection.execute(
                    ...
                    VALUES ('ws_alpha', 'project', 'amiga', 'CHAT-SAMEID', 'participant_codex',
                            'binding_legacy', 1, 'active', 1, 'provider_codex', 'revision_1',
                            'endpoint_codex', 'session_legacy', 'native_legacy', 'runtime_legacy', ?)
                    """,
                    (applied_at,),
                )
                connection.execute(
                    ...
                    VALUES ('ws_alpha', 'project', 'amiga', 'CHAT-SAMEID', 'participant_codex',
                            'challenge_legacy', 'pending', 'provider_codex', 'revision_1',
                            'endpoint_codex', 'session_legacy', 'native_legacy', 'runtime_legacy',
                            ?, ?, ?, NULL)
                    """,
                    ("a" * 64, "2026-07-23T00:01:00+00:00", applied_at),
                )
                for version, _statements in store_module.MIGRATIONS[:11]:
                    connection.execute(
                        """
                        INSERT INTO schema_migrations
                        (version, migration_checksum, applied_at_utc, tool_version, backup_reference)
                        ...
                        (
                            ...
                            applied_at,
                            MIGRATION_TOOL_VERSION,
                            f"ledger-{version - 1}-{stamp}.sqlite3",
                        ),
                    )
                connection.execute("PRAGMA user_version = 11")
            for version in range(11):
                backup = paths.backups / f"ledger-{version}-{stamp}.sqlite3"
                with closing(sqlite3.connect(paths.ledger)) as source, closing(
                    sqlite3.connect(backup)
                ) as destination:
                    source.backup(destination)
                ...
                with LedgerStore.open_writer(paths, clock=lambda: FIXED_TIME) as store:
                    ...
                    self.assertEqual(
                        store._connection.execute(
                            "SELECT session_ref_id, owner_key FROM conversation_bindings"
                        ).fetchone(),
                        ("session_legacy", None),
                    )
                    self.assertEqual(
                        store._connection.execute(
                            "SELECT agent_id FROM session_binding_challenges"
                        ).fetchone()[0],
                        "agent_codex",
                    )
    ...
    def test_exact_released_v1_is_writer_migrated_backed_up_and_reader_rejected(self) -> None:
        with TemporaryDirectory(dir="/tmp") as tmp:
            ...
            with LedgerStore.open_reader(paths) as reader:
                self.assertEqual(reader.schema_version(), SCHEMA_VERSION)
    ...
    def test_exact_released_v3_migrates_to_v7_and_failed_v4_restores_v3(self) -> None:
        with TemporaryDirectory(dir="/tmp") as tmp:
            paths = LedgerPaths.derive(tmp, "ws_alpha")
    ...
    def test_v7_observation_scheduler_cursor_is_scoped_validated_and_clearable(self) -> None:
        with TemporaryDirectory(dir="/tmp") as tmp:
            paths = LedgerPaths.derive(tmp, "ws_alpha")
            with LedgerStore.open_writer(paths, clock=lambda: FIXED_TIME) as store:
                record_test_registry(store)
                ...
                with self.assertRaisesRegex(ValueError, "snapshot is absent"):
                    store.advance_observation_scheduler_cursor(
                        ...
                        registry_revision="sha256:" + "b" * 64,
                        next_project_id=AMIGA,
                        updated_at_utc=FIXED_TIME.isoformat(),
                    )
                store.advance_observation_scheduler_cursor(
                    workspace_id="ws_alpha",
                    source_id="chats_mailbox",
                ...
                with self.assertRaisesRegex(ValueError, "does not own"):
                    store.observation_scheduler_cursor(
                        workspace_id="ws_other", source_id="chats_mailbox"
                    )
                with self.assertRaisesRegex(ValueError, "source_id"):
                    store.clear_observation_scheduler_cursor(
                        workspace_id="ws_alpha", source_id="other_source"
                    )

            with LedgerStore.open_reader(paths) as reader:
                with self.assertRaises(PermissionError):
                    reader.advance_observation_scheduler_cursor(
                        workspace_id="ws_alpha",
                        source_id="chats_mailbox",
                        registry_revision=REVISION,
                        next_project_id=AMIGA,
                ...
                with self.assertRaises(PermissionError):
                    reader.clear_observation_scheduler_cursor(
                        workspace_id="ws_alpha", source_id="chats_mailbox"
                    )
            ...
            with LedgerStore.open_writer(paths, clock=lambda: FIXED_TIME) as store:
                ...
                for name, values in invalid_rows.items():
                    with self.subTest(name=name), self.assertRaises(sqlite3.IntegrityError):
                        store._connection.execute(insert, values)
